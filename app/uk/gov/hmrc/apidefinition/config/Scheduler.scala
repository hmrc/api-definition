...
import uk.gov.hmrc.apidefinition.scheduled.DefaultCategoriesJob
...
@Singleton
class Scheduler @Inject()(defaultCategoriesJob: DefaultCategoriesJob,
  ...
  override lazy val scheduledJobs: Seq[ScheduledJob] = Seq(defaultCategoriesJob).filter(_.isEnabled)
}
