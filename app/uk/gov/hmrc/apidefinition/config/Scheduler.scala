...
import uk.gov.hmrc.apidefinition.scheduled.RenameContextJob
...
@Singleton
class Scheduler @Inject()(renameContextJob: RenameContextJob,
  ...
  override lazy val scheduledJobs: Seq[ScheduledJob] = Seq(renameContextJob).filter(_.isEnabled)
}
