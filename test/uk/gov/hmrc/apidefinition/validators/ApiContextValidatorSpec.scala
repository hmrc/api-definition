...
class ApiContextValidatorSpec extends AbstractValidatorSpec {
  ...
  "ApiContextValidator" when {
    ...
    "validateContext when skipping validation" should {
    ...
    "validateContext when not skipping validation" should {
      "succeed when the context has no empty path segments and matches the regular expression" in {
    ...
    "validateContextHasAtLeastTwoSegments when skipping validation" should {
    ...
    "validateContextHasAtLeastTwoSegments when not skipping validation" should {
      "succeed when the context has two segments" in {
    ...
    "validateForExistingAPI when skipping validation" should {
      ...
      "succeed with a good context" in {
        validates(ApiContextValidator.validateForExistingAPI(true)(ApiContext("individuals/foo")))
      }
    }

    "validateForExistingAPI when not skipping validation" should {
      "collect all errors when failing" in {
        val badContext = ApiContext("hello/my_world")
        failsToValidate(ApiContextValidator.validateForExistingAPI(false)(badContext), numberOfErrors = 2)(
      ...
      "succeed with a good context" in {
        validates(ApiContextValidator.validateForExistingAPI(false)(ApiContext("individuals/foo")))
      }
    }

    "validateForNewAPI when skipping validation" should {
    ...
    "validateForNewAPI when not skipping validation" should {
      "collect all errors when failing" in {
...
//     def verifyValidationPassed(result: validatorUnderTest.HMRCValidatedNel[ApiContext], expectedContext: ApiContext) = {
//       result.isValid should be(true)
//       val Valid(validatedContext) = result
//       validatedContext should be(expectedContext)
//     }

//     def verifyValidationFailed(result: validatorUnderTest.HMRCValidatedNel[ApiContext], expectedErrors: Seq[String]) = {
...
//       val result: validatorUnderTest.HMRCValidatedNel[ApiContext] = await(validatorUnderTest.validate(errorContext, apiDefinition)(context))
...
//       val result: validatorUnderTest.HMRCValidatedNel[ApiContext] = await(validatorUnderTest.validate(errorContext, apiDefinitionWithNewVersion)(context))
...
//       val result: validatorUnderTest.HMRCValidatedNel[ApiContext] = await(validatorUnderTest.validate(errorContext, apiDefinitionWithNewVersion)(newContext))
...
//         val result: validatorUnderTest.HMRCValidatedNel[ApiContext] = await(validatorUnderTest.validate(errorContext, apiDefinition)(badContext))
...
//       val result: validatorUnderTest.HMRCValidatedNel[ApiContext] = await(validatorUnderTest.validate(errorContext, newAPI)(newContext))
